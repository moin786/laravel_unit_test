...
namespace Peal\CorePackages\Utility\Classes;
...
class DataTable
{
	...
	static function limit ( $request, $columns )
	{
		...
		if ( isset($request['start']) && $request['length'] != -1 ) {
			//$limit = "WHERE sl >".intval($request['start'])." AND sl<=".intval($request['length']);
			$lenght = $request['length'];
			$start = $request['start'];
			$limit = "OFFSET $start ROWS FETCH NEXT ($lenght) ROWS ONLY";
		}
	...
	static function simple ( $request, $table, $primaryKey, $columns )
	{
		...
		// }
		$data = DB::select("SELECT ".implode(", ", self::pluck($columns, 'db'))."
				FROM $table
				$where
				$order
				$limit"
		...
		$resFilterLength = DB::select("SELECT COUNT({$primaryKey}) total
			 FROM   $table
			 $where"
		);
		...
		$resTotalLength = DB::select("SELECT COUNT({$primaryKey}) total
			 FROM   $table");
